...
function titleimage(src) {
  var titleimagediv = document.getElementsByClassName("title-img");

 for(j=0 ; j<titleimagediv.length;j++){
  titleimagediv[j].className="title-img"
 }

  event.target.classList.add("select");
  titleimageselect = src;
  console.log(titleimageselect)
}
  ...
  if (userName.value.trim() !== "") {
    if (postTitle.value.trim() !== "") {
     ...
     data-aos-duration="1500">
         ...
         <div >
         <div class= "text-uppercase text-center title-div" 
         style="border-bottom: 1px solid lightgrey  !important; color: rgb(7, 1, 92)!important;background:url(${titleimageselect}) !important">
